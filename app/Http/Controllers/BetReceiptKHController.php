    ...
    public function index(Request $request)
    {
        ...
        $data = DB::table('bet_kh')
            ->select(
                ...
                'bet_receipt_kh.id as receipt_id',
            ...
            ->join('bet_receipt_kh','bet_receipt_kh.id','=','bet_kh.bet_receipt_id')
            ...
            ->when(!is_null($date), function ($q) use ($date) {
                $q->where('bet_kh.bet_date', '>=', Carbon::parse($date)->startOfDay()->format('Y-m-d H:i:s'));
                $q->where('bet_kh.bet_date', '<=', Carbon::parse($date)->endOfDay()->format('Y-m-d H:i:s'));
            })
            ->when(!is_null($no), function ($q) use ($no) {
                $q->where('bet_receipt_kh.receipt_no', 'like', $no . '%');
            })
            ->groupBy(
                'users.id',
                'users.username',
            ...
            ->orderBy('bet_receipt_kh.receipt_no', 'asc')
            ->get();
...
public function getBetByReceiptId($id)
{
    ...
    $sumAmount = DB::table('bet_kh')
    ->select(
        DB::raw('SUM(bet_kh.total_amount) AS total_amount'), // typo fixed here: "totatlAmount" → "total_amount"
        DB::raw('SUM(bet_kh.total_amount * bet_package_configurations.rate / 100) AS net_amount')
    )
    ->join('bet_package_configurations', 'bet_package_configurations.id', '=', 'bet_kh.bet_package_config_id')
    ->where('bet_kh.bet_receipt_id', $id)
    ->groupBy('bet_kh.bet_receipt_id')
    ->first();
...
public function printReceiptNo($receiptNo)
    {
        $result = $this->model->with(['betsKH.betLotterySchedule', 'betsKH.betNumberKH','user'])
        ...
        ->join('bet_package_configurations', 'bet_package_configurations.id', '=', 'bet_kh.bet_package_config_id')
        ->where('bet_kh.bet_receipt_id', $result->id)
        ->groupBy('bet_kh.bet_receipt_id')
        ->first();
