...
class LottoBetKH extends Component
{
    protected $betModel;
    ...
    )
    {
        ...
        // VND users share the same AccountVND balance across Vietnam and Cambodia bets
        $this->betAccount = AccountVND::where('user_id', $this->user->id)->value('credit_balance') ?? 0;
    ...
    private function handleSimpleBet($number, $key)
    {
        ...
        switch ($length) {
            case 2:
                // 2D: A, B, C, D, ABCD, Roll, Roll2 enabled
    ...
    public function handleSave()
    {
        ...
        // VND users share AccountVND across Vietnam and Cambodia; USD would use AccountKHUSD (handled in KH-USD component)
        $AccountModel = AccountVND::class;
        $TransactionModel = CreditTransactionVND::class;
        ...
        try {
                        ...
                        if ($this->province_body_check[$key_prov][$key] && $this->total_amount[$key] > 0) {
                           ...
                           $lastAmount = $betAmountDupplicate ? $amountBet + 0.01 : $amountBet;
                            ...
                            $betNumber1 = [
                                ...
                                'c_amount' => $this->c_amount[$key] ?? 0,
                                'd_amount' => $this->d_amount[$key] ?? 0,
                                'abcd_amount' => $this->abcd_amount[$key] ?? 0,
                                'roll_amount' => $this->roll_amount[$key] ?? 0,
                                'roll2_amount' => $this->roll2_amount[$key] ?? 0,
                            ...
                            $betTypes = [
                                ...
                                'c' => [
                                    'amount' => $this->c_amount[$key] ?? 0,
                                    'check' => $this->c_check[$key],
                                ],
                                'd' => [
                                    'amount' => $this->d_amount[$key] ?? 0,
                                    'check' => $this->d_check[$key],
                                ],
                                'abcd' => [
                                    'amount' => $this->abcd_amount[$key] ?? 0,
                                    'check' => $this->abcd_check[$key],
                                ],
                                'roll' => [
                                ...
                                'roll2' => [
                                    'amount' => $this->roll2_amount[$key] ?? 0,
                                    'check' => $this->roll2_check[$key],
                                ],
                                'roll_parlay' => [
            ...
            DB::rollBack();
            Log::error('KH bet save failed', ['error' => $e->getMessage(), 'trace' => $e->getTraceAsString(), 'user_id' => auth()->id()]);
                ...
                if ($schedule->code == "HN") {
                    ...
                    if ($this->abcd_amount[$key] > 0) {
                        $ab = match ($lengthNumber) {
                            2 => MultiplierHNKHEnum::ABCD,
                            3 => MultiplierHNKHEnum::ABCD_3D,
                            default => 1
                        };
                        if ($this->abcd_check[$key]) {
                            $this->totalProvisionalHN += $this->abcd_amount[$key] * $ab * $this->permutationsLength[$key];
                        } else {
                            if ($isAsterisk) {
                                $this->totalProvisionalHN += $this->abcd_amount[$key] * $ab * 10;
                            } else {
                                $this->totalProvisionalHN += $this->abcd_amount[$key] * $ab;
                            }
                        }
                    }
                    ...
                    if ($this->roll_amount[$key] > 0) {
                        $roll = match ($lengthNumber) {
                            2 => MultiplierHNKHEnum::ROLL,
                            3 => MultiplierHNKHEnum::ROLL_3D,
                            4 => MultiplierHNKHEnum::ROLL_4D,
                            default => 1
                        };
                    ...
                    if ($this->roll2_amount[$key] > 0) {
                        $roll2Multiplier = match ($lengthNumber) { 2 => MultiplierHNKHEnum::ROLL2, 3 => MultiplierHNKHEnum::ROLL2_3D, default => 0 };
                    ...
                    if ($this->abcd_amount[$key] > 0) {
                        if ($this->abcd_check[$key]) {
                            $this->totalProvisional += ($this->abcd_amount[$key] * MultiplierKHEnum::ABCD) * $this->permutationsLength[$key];
                        } else {
                            if ($isAsterisk) {
                                $this->totalProvisional += $this->abcd_amount[$key] * MultiplierKHEnum::ABCD * 10;
                            } else {
                                $this->totalProvisional += $this->abcd_amount[$key] * MultiplierKHEnum::ABCD;
                            }
                        }
                    }
                    if ($this->roll_amount[$key] > 0) {
                        $roll = match ($lengthNumber) {
                            2 => MultiplierKHEnum::ROLL,
                            3 => MultiplierKHEnum::ROLL_3D,
                            4 => MultiplierKHEnum::ROLL_4D,
                            default => 1
                        };
                        if ($this->roll_check[$key]) {
                    ...
                    if ($this->roll2_amount[$key] > 0) {
                        $roll2Multiplier = match ($lengthNumber) { 2 => MultiplierKHEnum::ROLL2, 3 => MultiplierKHEnum::ROLL2_3D, default => 0 };
                        if ($this->roll2_check[$key]) {
                            $this->totalProvisional += $this->roll2_amount[$key] * $roll2Multiplier * $this->permutationsLength[$key];
                        } else {
                            $this->totalProvisional += $isAsterisk
                                ? $this->roll2_amount[$key] * $roll2Multiplier * 10
                                : $this->roll2_amount[$key] * $roll2Multiplier;
