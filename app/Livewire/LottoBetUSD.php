...
class LottoBetUSD extends Component
{
    ...
    {
        return view('livewire.lotto-bet-usd');
    }
                        ...
                        if ($this->province_body_check[$key_prov][$key] && $this->total_amount[$key] > 0) {
                           ...
                           $lastAmount = $betAmountDupplicate ? $amountBet + 0.01 : $amountBet;
                            ...
                            $betNumber1 = [
                                ...
                                'ab_amount' => (float)($this->ab_amount[$key] ?? 0),
                                'roll_amount' => (float)($this->roll_amount[$key] ?? 0),
                                'roll7_amount' => (float)($this->roll7_amount[$key] ?? 0),
        ...
        if (strpos($number, '#') !== false) {
            ...
            foreach ($betTypes as $info) {
                if ($info['amount'] > 0) {
                        ...
                        + COALESCE(SUM(bet_number_usd.ab_amount),0) 
                        + COALESCE(SUM(bet_number_usd.roll_amount),0) 
                        + COALESCE(SUM(bet_number_usd.roll7_amount),0) 
                        + COALESCE(SUM(bet_number_usd.roll_parlay_amount),0) as total
                        ')
                        ->value('total');
                        if ($checkBetLimit) {
