        ...
        <div class="bp-filter">
            ...
            <select id="digit_type" style="width:100px;">
                @foreach ($digits as $val)
                    ...
                    @endphp
                    <option value="{{ $optionValue }}" {{ $digit_type == $optionValue ? 'selected' : '' }}>{{ $optionValue }}</option>
                @endforeach
            </select>
            <input type="text" id="number" value="{{ $number }}" style="width:120px;" placeholder="{{ __('message.number') }}">
            <button class="bp-search-btn" onclick="searchReceipt('{{ route('bet.bet-number') }}')">
                ...
                <tbody>
                    @if (isset($data) && count($data) > 0)
                        ...
                        <tr>
                            ...
                            <td class="bp-num" style="background:#f8f9fc;border-top:2px solid #e2e8f0;font-weight:700;{{ $totalNetAmount['win_lose'] < 0 ? 'color:#dc2626;' : '' }}">{{ number_format($totalNetAmount['win_lose'], 3, '.', '') }}</td>
                        </tr>
                    @else
