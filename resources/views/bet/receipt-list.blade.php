                        ...
                        @foreach ($data as $key => $row)
                            ...
                            <tr class="{{ $row->compensate > 0 ? 'bp-win' : '' }}">
                                ...
                                <td onclick="handleShowBet('{{ $row->receipt_id }}')" class="bp-center">
                                    <a href="#" onclick="return false;" style="color:#2563eb;font-weight:600;">
                                        {{ $row->receipt_no ?? '' }}
                                    </a>
                                </td>
                                <td>{{ $row->account ?? '' }}</td>
                                <td>{{ $row->bet_date ?? '' }}</td>
                                <td class="bp-center">VND</td>
            ...
            <div class="relative bg-white rounded-lg shadow-sm" style="border:1px solid #e2e8f0;">
                <div class="flex items-center justify-between p-4 border-b border-gray-200">
                    <h6 style="font-size:.95rem;font-weight:700;color:#1e293b;margin:0;">
                        {{ __('message.receipt_no') }}: <span id="receipt_no" style="color:#2563eb;"></span>
                    </h6>
<button type="button"
                        style="background:none;border:none;cursor:pointer;color:#6b7280;padding:4px;"
                        onclick="closeReceiptModal()">
                        <svg style="width:18px;height:18px;" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
...
<button type="button" onclick="printReceipt()"
                        style="background:#1e3a8a;color:#fff;border:none;border-radius:8px;padding:8px 18px;font-weight:600;cursor:pointer;">{{ __('Print') }}</button>
<button type="button" onclick="closeReceiptModal()"
    ...
    <script>
        ...
        function handleShowBet(id) {
            ...
            fetch(`/lotto_vn/bet/${id}`)
                ...
                .then(data => {
                    ...
                    document.getElementById('totalAmount').innerText = Number(data?.totalAmount).toFixed(2) + ' (VND)';
                    document.getElementById('dueAmount').innerText = Number(data?.dueAmount).toFixed(2) + ' (VND)';
                })
        ...
        function printReceipt() {
            ...
            const w = window.open('/lotto_vn/bet_receipt/' + receiptNo, '_blank');
