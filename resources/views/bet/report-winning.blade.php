        ...
        <div class="bp-filter">
            ...
            <select id="company" style="width:160px;">
                @foreach ($companies as $val)
                    <option value="{{ $val['id'] }}" {{ $company == $val['id'] ? 'selected' : '' }}>{{ $val['label'] }}</option>
                @endforeach
            </select>
            <input type="text" id="bet-number" value="{{ $number }}" style="width:140px;" placeholder="{{ __('message.number') }}">
<button class="bp-search-btn" onclick="searchWinning('{{ route('bet.bet-winning') }}')">
            ...
            <button class="bp-search-btn" style="background:#6b7280;" onclick="clearSearch('{{ route('bet.bet-winning') }}')">
                {{ __('message.clear') }}
            </button>
        </div>
        ...
        <div class="bp-table-wrap" style="padding:0 0 8px;">
            <table class="bp-table">
                ...
                <tbody>
                    @if (isset($data) && count($data))
                        ...
                        @foreach ($data as $key => $row)
                            <tr>
                                ...
                                <td class="bp-center">{{ $row['original_number'] ?? '' }}</td>
                        ...
                        @endforeach
                        <tr>
                            <td colspan="14" style="background:#f8f9fc;border-top:2px solid #e2e8f0;"></td>
                    ...
                    @else
                        <tr><td colspan="17" class="bp-empty">
    ...
    <script>
        function searchWinning(url) {
            ...
            if (date.length) {
                ajaxLoad(url + '?date=' + date + '&number=' + number + '&company=' + company);
            }
        }
