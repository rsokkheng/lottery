<x-app-layout>
                        ...
                        @foreach ($data as $key => $row)
                            ...
                            <tr class="{{ $row->compensate > 0 ? 'bp-win' : '' }}">
                                ...
                                <td onclick="handleShowBet('{{ $row->receipt_id }}')" class="bp-center">
                                    <a href="#" onclick="return false;" style="color:#a17100;font-weight:600;">
                                        {{ $row->receipt_no ?? '' }}
                                    </a>
                                </td>
                                <td>{{ $row->account ?? '' }}</td>
                                <td>{{ $row->bet_date ?? '' }}</td>
                                <td class="bp-center">{{ $currencyLabel }}</td>
            ...
            <div class="relative bg-white rounded-lg shadow-sm" style="border:1px solid #e2e8f0;">
                <div class="flex items-center justify-between p-4 border-b border-gray-200">
                    <h6 style="font-size:.95rem;font-weight:700;color:#1e293b;margin:0;">
                        {{ __('message.receipt_no') }}: <span id="receipt_no" style="color:#a17100;"></span>
                    </h6>
                ...
                <div class="flex justify-end gap-2 p-4 border-t border-gray-200">
                    ...
                    <button type="button" onclick="printReceipt()"
                        style="background:#7d5a00;color:#fff;border:none;border-radius:8px;padding:8px 18px;font-weight:600;cursor:pointer;">{{ __('Print') }}</button>
    ...
    <script>
        ...
        function handleShowBet(id) {
            ...
            fetch(`{{ $urlBetDetail }}/${id}`)
                ...
                .then(data => {
                    ...
                    document.getElementById('totalAmount').innerText = Number(data?.totalAmount).toFixed(2) + ' ({{ $currencyLabel }})';
                    document.getElementById('dueAmount').innerText = Number(data?.dueAmount).toFixed(2) + ' ({{ $currencyLabel }})';
                })
        ...
        function printReceipt() {
            ...
            const w = window.open('{{ $urlBetReceipt }}/' + receiptNo, '_blank');
