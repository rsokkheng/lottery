<div>
    ...
    <div class="bg-white shadow-md p-4 rounded-lg">
        ...
        {{-- ── System badge ── --}}
        <div class="inline-flex items-center gap-2 mb-3 px-3 py-1 rounded" style="background:linear-gradient(135deg,#1e3a8a 0%,#2563eb 100%)">
            <span class="text-white font-bold text-sm tracking-wide">BET VIETNAM</span>
            <span class="text-xs font-semibold px-2 py-0.5 rounded" style="background:rgba(255,255,255,.22);color:#fff;">VND</span>
        </div>

        {{-- ── Stat cards row ── --}}
        <div class="grid grid-cols-2 gap-2 mb-3 sm:grid-cols-4">
            {{-- Credit --}}
            <div class="rounded-lg p-2.5 border {{ $totalDue > $betAccount ? 'border-red-300 bg-red-50' : 'border-blue-100 bg-blue-50' }}">
                <div class="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-0.5">{{ __('lang.bet-credit') }}</div>
                <div class="font-bold text-sm {{ $totalDue > $betAccount ? 'text-red-600' : 'text-blue-700' }}">
                    {{ number_format($betAccount ?? 0, 2) }} <span class="font-normal text-xs">VND</span>
                </div>
            </div>
            {{-- Outstanding --}}
            <div class="rounded-lg p-2.5 border border-gray-100 bg-gray-50">
                ...
                <div class="font-bold text-sm text-gray-700">
                    {{ number_format($totalOutstanding, 2) }} <span class="font-normal text-xs">VND</span>
                </div>
            </div>
            {{-- Total amount --}}
            <div class="rounded-lg p-2.5 border border-indigo-100 bg-indigo-50">
                ...
                <div class="font-bold text-sm text-indigo-700">
                    {{ number_format($totalInvoice, 2) }} <span class="font-normal text-xs">VND</span>
                </div>
            </div>
            {{-- Total due --}}
            <div class="rounded-lg p-2.5 border {{ $totalDue > $betAccount ? 'border-red-300 bg-red-50' : 'border-green-100 bg-green-50' }}">
                ...
                <div class="font-bold text-sm {{ $totalDue > $betAccount ? 'text-red-600' : 'text-green-700' }}">
                    {{ number_format($totalDue, 2) }} <span class="font-normal text-xs">VND</span>
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center gap-2 bg-red-50 border border-red-300 text-red-700 rounded-lg px-3 py-2 mb-3 text-xs font-semibold">
            ...
            Insufficient credit — need <span class="font-extrabold mx-1">{{ number_format(round($totalDue - $betAccount, 2), 2) }} VND</span> more.
        </div>
        @endif
