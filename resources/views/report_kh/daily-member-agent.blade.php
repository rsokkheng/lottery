<x-app-layout>
    ...
    @php $agentUrl = route($khRoutePrefix . '.reports.daily-member-agent', ['id' => $memberId]); @endphp
    <div class="bp-wrap">
        <div class="bp-filter">
            ...
            <button class="bp-search-btn" onclick="searchReceipt('{{ $agentUrl }}')">
            ...
            <button class="bp-search-btn" style="background:#6b7280;" onclick="clearSearch('{{ $agentUrl }}')">
                {{ __('message.clear') }}
            </button>
            <a href="{{ route($khRoutePrefix . '.reports.daily-manager') }}"
                ...
                ← {{ __('message.back') }} &nbsp;<span style="opacity:.85;">{{ $managerName->username }}</span>
            </a>
        </div>
    ...
    <script>
        function searchReceipt(url) {
            ...
            if (date.length) { ajaxLoad(url + '?date=' + date + '&com_id=' + com_id); }
        }
